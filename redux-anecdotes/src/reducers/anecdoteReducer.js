...
const anecdoteSlice = createSlice({
	...
	reducers:{
		updateVote(state,action){
			...
			const newList = state.map(item => item.id === updatedAnecdote.id ? updatedAnecdote : item)
...
export const vote = (anecdote) => {
	return async dispatch =>{
		const returnedAnecdote = await anecdoteService.voteAnecdote(anecdote.id,anecdote)
